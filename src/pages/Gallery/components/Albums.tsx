import { Box, Image, Flex, Text, chakra, shouldForwardProp } from '@chakra-ui/react';
...
function Albums() {
 ...
 // DYNAMIC DATA
 const dataImr: galleryProp = importedData;
   ...
   <Box width={{ lg: '80%', sm: '90%', base: '90%' }} margin={'auto'} py={8}>
    <MotionBox initial={{ y: 100 }} animate={{ y: 0 }} transition={customeTranstion}>
     <Flex
      ...
      {Object.keys(dataImr).map((item: any) => {
       const ArrayImages = dataImr[item];
          ...
          <Box sx={cardAlbum} className="single_card_album">
           <Flex direction={'column'} gap={5}>
            ...
            <Box className="single_album_text">
             <Text sx={textAlbum}>{ictVerson[item as keyof typeof ictVerson]}</Text>
            </Box>
           </Flex>
          </Box>
         </Link>
